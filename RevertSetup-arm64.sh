...
sudo docker rmi zigbee2mqtt/zigbee2mqtt-edge-aarch64:latest \
  openhab/openhab:3.4.0-snapshot-alpine \
  deconzcommunity/deconz \
  portainer/portainer-ce:linux-arm64 \
  sepia/home \
  eclipse-mosquitto sepia/stt-server:vosk_aarch64 &> /dev/null
echo -e "${GREEN}Complete!$NC"
